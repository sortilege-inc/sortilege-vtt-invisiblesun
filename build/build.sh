#!/usr/bin/env bash
# Regenerate data/ from the Invisible Sun DSL corpus and prove the round trip.
#
#   bash build/build.sh [<path to titterpig-dsl-invisiblesun/0.5>]
#
# build (every token of every file consumed, or the parser raises; every corpus file
# claimed by exactly one book, or the build raises) → verify both directions → node --check
# every data file. Any failure exits non-zero.
set -euo pipefail
cd "$(dirname "$0")/.."
CORPUS="${1:-$HOME/Sortilege/Titterpig/DSL/titterpig-dsl-invisiblesun/0.5}"
