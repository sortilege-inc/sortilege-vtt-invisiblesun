...
**D6 — Domain.** `invisiblesun.sortilege.online` via a Pages CNAME.
- **(owner) Decided:** that is the name. Adding the CNAME through the Pages UI is the owner's step
  at M6; until then the github.io address serves. The plan assumes both origins will be live, so
  nothing hard-codes one.
