...
| G1 | **The GM page** (`gm/index.html`, `system/invisiblesun/{ops,table,panels}.js`, `assets/css/invisiblesun-gm.css`): the engine's shell over Invisible Sun's panels — **Narrative** (the mode in play from The Gate p7; the GM's own scenes: add, arrange by drag, done, notes, who is in it), **Party**, **Inspector**, **Bestiary**, **Rules & Books**, **Log**, **Narrative pack**. The system's ops (`narrative`, `scenes`, `sooth`) are a UMD module beside the engine's | **landed 2026-09-20** — browser, through the real controls: the shell opens on Narrative · Party · Inspector with base + Key + Way + Gate + decks loaded (5 files) and 0 console errors; two scenes typed and added, *Action* chosen, the first scene current; the Bestiary fetched Teratology on open and listed **193 stat blocks** (Teratology 190 + The Way 3) by book and sun, "demon" narrowed them, a block opened in the Inspector and **Put in The Chapel of the Unwelcome Child** made it the scene's cast and a token source; Rules & Books searched "bene" → 120 results with excerpts, the scope offering all 15 books with fetch-on-pick. Under node, the system ops apply identically (`putScene`, `setSceneCast`, three `soothTurn`s, `soothClear`) and the role rules hold: the GM may turn a card, a player may not, a player may change only their own live state |
| G2 | **The live sheet and the roll** (`IsSheet.live/readMember/downloadMember/doRoll`): a character file becomes a party member typed as the corpus's ACTOR; pools as bene spent against the sheet's division; the six harm/advancement values; the d10 roll as the book states it (challenge − venture, enhancements as extra dice, one bene per action) logged to the shared log | **landed 2026-09-20** — browser: Ossian Vahl loaded from a file (`templateId` = `#isunVislae000000001`), the Inspector showed his sentence, eight pools at n / n, six counters; *spend* on Accuracy read 1 / 2; a roll at challenge 5, venture 1, a bene from Physicality and +1 die from Sortilege read *needs 3 (challenge 5 − venture 2)*, dice 10 · 10, success, and the three pools were debited |
| G3 | **The Path of Suns** (`system/invisiblesun/sooth.js`, `gm/path.html`): the sixty `^"Sooth Card"` entities on the book's path, shared state and ops (shuffle, turn, clear, reset); the panel with the active card's effect on magic and on each vislae's venture, and its own listing; the board for a second screen (`?view=player` hides the meanings) | **landed 2026-09-20** — browser: *Turn a card on Silver*, three turns filled Silver · Green · Blue with the active card on Blue; *Rat* (Companion, Mysteries) read its royalty rule and *+1 venture for Ossian Vahl* (a Stoic, The Key p67); nine turns filled the path and the Invisible Sun (*Imperator*), the tenth began on Silver with one card on the path and *In the Testament of Suns · Imperator* kept; the board showed the same nine slots, and `?view=player` showed the effects and meanings-chips with 0 prose paragraphs. *Endless Maze* carries `[Blue, Red]` — enhanced first, diminished second, as The Gate p10 says; the 24 royalty cards carry no suns |
| G4 | **The table and the player's page** (`gm/vtt.html`, `gm/play.html`): the engine's map table over the GM's scenes (no shipped maps), tokens from the party and the scene's cast; the player's page with the active card above the sheet | **landed 2026-09-20** — browser: the table opened on *The Chapel of the Unwelcome Child*, listed both scenes, offered *Ossian Vahl* and the scene's cast as tokens; the player's page showed *Join the table* pointing at the local Worker. 0 console errors on all three pages |
| G5 | **Sessions** (`worker/`: the SessionRoom, importing the engine's ops **and** `system/invisiblesun/ops.js`; `ALLOWED_ORIGIN` = actuality + github.io) | **landed 2026-09-20** — `wrangler dev` on 8787 (launch entry `vtt-invisiblesun-worker`); the GM's *Start session* went live as room **WZHZX**; a player at a second origin (127.0.0.1) took the room's snapshot (the party, both scenes, the path), claimed Ossian, saw the sheet **without** GM notes and **with** the active card's strip, rolled *read the sooth* at challenge 4 with a bene from Intellect → *needs 3*, a 7, success — and the GM's page had the roll and the debited pool within a second; the GM turned *Questing Knight* on Green and the player's strip showed it, meanings hidden. **Deploy is the owner's step:** `cd worker && npx wrangler deploy`, then `engine/config.js` `worker.deployed` |
...
| 26 | **(owner)** With no modules in the corpus, a session is organised by **GM-authored scenes and the mode in play** (Narrative panel): scenes are the GM's own (`scenes` op), done/notes/current use the engine's scene ops under one module id, `narrative`; the mode's names are The Gate's (p7) and each links to its rule | The corpus has no scene or cast structure — the sourcebooks are setting and rules, Teratology's creatures are prose stat blocks. The Gate itself leaves scenes to the GM ("Scenes or Encounters", p65). |
| 27 | **(owner)** The Path of Suns is a **panel and a board window** (`gm/path.html`); `?view=player` keeps the card, its suns and its effect on magic and hides the GM's meanings; the player's page carries the active card above the sheet | The Sooth Deck is the table in this game. Which sun is enhanced is the card's own field order (`Suns[0]` bold, `Suns[1]` faint — The Gate p10, checked against *Endless Maze*); the path order is the corpus's `^"Color"` ENUM, which is the book's; the heart↔family pairings are a constant citing The Key p67. Nothing else about a card is decided here. |
...
page from `127.0.0.1`, since `localhost` shares the GM's stored session.
